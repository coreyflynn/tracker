...
// tell hoodie to update the todo store every second
setInterval(function(){
  hoodie.store.findAll('todo').then(function(allTodos) {
    allTodos.forEach(function (todo) {
      if (todos.inCollection(todo)) {
        todos.update(todo);
      }else{
        todos.add(todo);
        // todos.collection.push(todo);
      }
    });
  });
},1000);
...
function Todos($element) {
  ...
  this.inCollection = function(todo) {
    if (getTodoItemIndexById(todo.id) !== null) {
      return true
    }else{
      return false
    }
